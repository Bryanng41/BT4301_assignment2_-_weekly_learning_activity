"""Baseline: Logistic regression on classification event table."""
...
if __name__ == "__main__":
    ...
    tr_ev, te_ev = load_or_build_event_tables(
        _data, n_neg=5, random_state=42, always_rebuild=True
    )
    ...
    pipeline, th, _ = fit_clf("log_reg", tr_ev)
    ...
    mlflow.set_experiment("Assignment2 - Baseline Model")
    with mlflow.start_run():
        ...
        log_sklearn_pipeline_model(
            ...
            registered_model_name="assignment2_baseline_lr",
        )
        mlflow.log_metric("accuracy", acc)
        mlflow.log_metric("precision", prec)
        mlflow.log_metric("recall", rec)
        mlflow.log_metric("f1", f1)
        mlflow.set_tag("Training Info", "Logistic regression + OHE/scale (purchase yes/no)")
